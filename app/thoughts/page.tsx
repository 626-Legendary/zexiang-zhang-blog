...
function normalizeUrls(row: Pick<Thought, "image_url" | "image_urls">): string[] {
...
export default async function ThoughtsPage() {
  ...
  const [{ data: likeCounts }, { data: commentCounts }] = await Promise.all([
    ids.length
      ? supabase.from("like_counts").select("feed_id, like_count").eq("feed_kind", "thoughts").in("feed_id", ids)
      : Promise.resolve({ data: [] as any }),
    ids.length
      ? supabase.from("comment_counts").select("feed_id, comment_count").eq("feed_kind", "thoughts").in("feed_id", ids)
      : Promise.resolve({ data: [] as any }),
  ]);
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 py-10">
      <header className="mb-8">
        <h1 className="text-2xl font-semibold tracking-tight">随想</h1>
        <div className={ZMX.className}>

          <p className="mt-1 text-xl text-muted-foreground">心有所感，笔随其行</p>
        </div>
          ...
          thoughts.map((t) => {
            ...
            return (
              <article key={t.id} className="p-5">
                ...
                <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <form action={toggleLikeAction}>
                      <input type="hidden" name="kind" value="thoughts" />
                ...
                <div className="mt-5">
                  <CommentThread kind="thoughts" feedId={t.id} roots={roots} repliesByParent={repliesByParent} />
                </div>
              </article>
            );
          })
        ) : (
          <p className="p-6 text-sm text-muted-foreground">No public thoughts yet.</p>
        )}
      </section>
    </main>
  );
